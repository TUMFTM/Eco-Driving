...
%Vspace
sim.vspace.v2 = permute(repmat(sim.vspace.v2,1,1,in.t_res),[3 1 2]);
sim.vspace.dt = permute(repmat(sim.vspace.dt,1,1,in.t_res),[3 1 2]);
sim.vspace.E  = permute(repmat(sim.vspace.E ,1,1,in.t_res),[3 1 2]);

%Vbounds
sim.bounds.v2 = permute(repmat(sim.bounds.v2,1,1,in.t_res),[3 1 2]);
sim.bounds.dt = permute(repmat(sim.bounds.dt,1,1,in.t_res),[3 1 2]);
sim.bounds.E  = permute(repmat(sim.bounds.E ,1,1,in.t_res),[3 1 2]);

%% DP backward
v_space{length(bounds.s)} = 0;
Cost{length(bounds.s)} = zeros(1,in.t_res);
...
for i = length(bounds.s)-1:-1:1
    ...
    t2 =  repelem(bounds.t_space(i,:)',1,length(v_space{i}),in.a_res) + dt;
