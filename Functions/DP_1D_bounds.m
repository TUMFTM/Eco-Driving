function bounds = DP_1D_bounds(Route)
...
s_corner = round(Route.s_corner/ds)*ds; %adjust for distance step resolution
...
%acceleration phases
for i = 1:length(Iacc) 
    v_matrix(i,Iacc(i):Iacc(i)+length(s)-1) = sqrt(vlim(Iacc(i))^2+2*amax*s);
end

%deceleration phases
for i = 1:length(Idec)
    v_matrix(i+length(Iacc)+1,Idec(i)-length(s)+1:Idec(i))= sqrt(vlim(Idec(i))^2+2*amax*(s(end)-s));
end

v_matrix = v_matrix(:,1:length(bounds.s)); %Cut off length added by too long acc or dec profiles
bounds.v = fix(min(v_matrix)*10^rounding)*10^-rounding; %Derive speed bound
