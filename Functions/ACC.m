function [s_new, t_new, v_new] = ACC(s1, t1, v1, s2, t2, v2, Route)
...
diff_smin = s_intervehicle(s1,t1,v1,v2(1),Route,'min');
diff_smax = s_intervehicle(s1,t1,v1,v2(1),Route,'max');

%Adjust speed if the limits are violated
if diff_smin<0 %Lower distance limit is violated
    vmax = sqrt(v1^2+2*amax*ds);    
    f_smin = @(v2) s_intervehicle(s1,t1,v1,v2,Route,'min');
    ...
    if v_new == 0
        ...
        if t_pass>t1+2*ds/v1
            s_new = [s1 s1] + ds;
            t_new = [t1+2*ds/v1 t_pass];
            v_new = [0 0];
        else
            s_new = s1 + ds;
            t_new = t1+2*ds/v1;
        ...
        s_new = s1+ds;
        t_new = t1+2*ds./(v1+v_new);
    end
elseif diff_smax>0 %Upper distance limit is violated
    f_smax = @(v2) s_intervehicle(s1,t1,v1,v2,Route,'max');
    vmax = sqrt(v1^2+2*amax*ds);
    vmin = sqrt(v1^2-2*amax*ds);
    try
        v_new = min([max(Route.v_vlim) fzero(f_smax,[vmin vmax])]);       
    catch
        v_new = min([max(Route.v_vlim) sqrt(v1.^2+2*amax*ds)]);
    end
    s_new = s1+ds;
    t_new = t1+2*ds./(v1+v_new);
